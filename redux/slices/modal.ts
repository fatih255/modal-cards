...
export const ModalSlice = createSlice({
  ...
  reducers: {
    ...
    selectModal: (state, action: PayloadAction<ModalAlias>) => {
      ...
      state.contents = {
        ...selectedModalConstants.contents,
        texts: linkParser(selectedModalConstants.contents.texts),
      }
    ...
    selectRadioButton: (state, action: PayloadAction<number>) => {
      state.contents.radios.forEach((radio, index) => {
        if (radio.selected) {
          delete state.contents.radios[index].selected
        }
      })
    ...
    updateLayout: (
      state,
      action: PayloadAction<{ name: string; value: string | object }>,
    ) => {
      state.layout = {
        ...state.layout,
    ...
    updateModalContentText: (
      ...
      const currentValue =
        state.contents.texts[Number(action.payload.ContentIndex)]
      const haveLink = currentValue.includes('@Link')
      state.contents.texts[Number(action.payload.ContentIndex)] = ` ${
        action.payload.ContentText
      } ${haveLink ? '@Link:' + currentValue.split('@Link')[1] : ''}`
    },
    updateModalLinkURL: (
      state,
      action: PayloadAction<{ ContentIndex: string | number; LinkURL: string }>,
    ) => {
      const currentValue =
        state.contents.texts[Number(action.payload.ContentIndex)]
      const text = currentValue.split('@Link')[0]
      state.contents.texts[
        Number(action.payload.ContentIndex)
      ] = `${text} @Link:${action.payload.LinkURL}`
    },
    updateModalPostURL: (
      state,
      action: PayloadAction<{ ContentIndex: string | number; PostURL: string }>,
    ) => {
      const currentValue =
        state.contents.texts[Number(action.payload.ContentIndex)]
      ...
      state.contents.texts[
        Number(action.payload.ContentIndex)
      ] = `${text} @Post:${action.payload.PostURL}`
    },
    updateRadioButton: (
      state,
      action: PayloadAction<{
