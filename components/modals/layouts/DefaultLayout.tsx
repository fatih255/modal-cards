...
export default function DefaultLayout({ children }: layoutTypes) {
  const { size, position, className } = useAppSelector(
    (state) => state.modal.layout,
    shallowEqual,
  )
  ...
  return (
    <div
      ...
      <button
        className='close-modal-action no-w-full absolute top-0 right-0 hover:scale-110  trans-300 m-[6%]'
