...
export default function AfterPercentageScroll() {
  const { afterPercentageScroll, isAfterPercentageScroll } = useAppSelector(
    (state) =>
      Object({
        afterPercentageScroll: state.modal.settings.afterPercentageScroll,
        isAfterPercentageScroll: state.modal.activedSettings.includes(
  ...
  return (
    <Switch
      ...
      <InputText
        ...
        onChange={(value) =>
          dispatch(
            updateSettings({ name: 'afterPercentageScroll', value: value }),
          )
        }
