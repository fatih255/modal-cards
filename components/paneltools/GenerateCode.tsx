    ...
    <>
      ...
      <div
        ...
        <SyntaxHighlighter
          language='javascript'
          style={dark}
          customStyle={{ borderRadius: 8, height: 260, width: 'inherit' }}>
          {codeString}
        </SyntaxHighlighter>
        <div className='h-[60px] flex justify-end items-end '>
          <Button
            onClick={(e) => copyCodeHandler(e)}
            className='rounded-full py-1.5 !self-end'
            text='Copy the code'
          />
        </div>
      </div>
      <div
        className={cn(
          { hidden: !(codeString.length > 0) },
          'flex justify-start gap-2 items-start mt-[17px] w-[91%] ',
        )}>
        <div className=' '>
