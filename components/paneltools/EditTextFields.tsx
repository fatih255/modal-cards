...
export default function EditTextFields() {
  ...
  const texts = useAppSelector(
    (state) => state.modal.contents.texts,
    shallowEqual,
  )

  const onContentTextChangeHandler = (
    ...
    <>
      {(texts as LinkParserType).map((text, index: string | number) => (
        <React.Fragment key={`text-content-${index}`}>
          <InputText
            onChange={async (value) => onContentTextChangeHandler(index, value)}
            text={typeof text !== 'string' ? text.text : (text as string)}
          />
          {typeof text === 'object' && (
            <>
              {typeof text.linkURL === 'string' && (
                <InputLink
                  iconName='AiOutlineLink'
                  onChange={(value) => onLinkChangeHandler(index, value)}
                />
              )}
              {typeof text.postURL === 'string' && (
                <InputLink
                  iconName='BsSignpost'
                  onChange={(value) => onPostURLChangeHandler(index, value)}
                />
              )}
            </>
          )}
