...
export default function TrafficSource() {
  const { trafficSource, isActiveTrafficSource } = useAppSelector(
    (state) =>
      Object({
        trafficSource: state.modal.settings.trafficSource,
        isActiveTrafficSource:
          state.modal.activedSettings.includes('trafficSource'),
      }),
    shallowEqual,
  )
  ...
  return (
    <Switch
      ...
      <InputText
        text={trafficSource}
        onChange={(value) =>
          dispatch(updateSettings({ name: 'trafficSource', value: value }))
        }
