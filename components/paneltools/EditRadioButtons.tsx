...
function EditRadioButtons({ }: Props) {
    ...
    const activeRadioTabHandler = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>, radioIndex: number) => {
        const showingTab = document.querySelector('.radio-tab-show') as HTMLElement
        const activeTabBtn = document.querySelector('.selected-radio-tab-btn') as HTMLElement
        ...
        const radioTab = document.querySelector(`.radio-tab-${radioIndex}`) as HTMLElement
    ...
    const onContentRadioChangeHandler = ({ radioIndex, title, description, value }: {
        radioIndex: number
        title: string
        description: string
        value: string
    ...
    return (
        ...
        radios ? <>
            ...
            {
                radios.map((radio, index) => <React.Fragment key={`text-radio-${index}`}>
                    <div className="flex flex-col">
                        <Button theme="light-bordered"
                            className={` ${index === 0 ? 'selected-radio-tab-btn' : ''} radio-tab-btn bg-design-gray-100 text-sm w-full my-0.5`}
