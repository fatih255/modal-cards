...
export default function VisitorDeviceSelect() {
  const { visitorDevice, isActiveVisitorDevice } = useAppSelector(
    (state) =>
      Object({
        visitorDevice: state.modal.settings.visitorDevice,
        isActiveVisitorDevice:
          state.modal.activedSettings.includes('visitorDevice'),
      }),
    shallowEqual,
  )
  ...
  return (
    <div>
      <Switch
        ...
        <CheckBoxButton
          returnedValue={(data) => {
            dispatch(updateSettings({ name: 'visitorDevice', value: data }))
          }}
          items={[
            {
              ...
              text: 'Desktop',
              value: 'desktop',
              icon: <DesktopIcon fill='#999999' />,
            },
            {
              checked: visitorDevice.includes('mobile'),
              text: 'Mobile',
              value: 'mobile',
              icon: <MobileIcon fill='#999999' />,
            },
          ]}
        />
      </Switch>
