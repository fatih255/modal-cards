...
export default function RadioButton({ options, returnedValue, defaultValue }: RadioButtonProps) {
        ...
        <div className="flex flex-row bg-design-gray-100 rounded-xl ">
            {
                options.map(({ text, value }, index) => (
                    <button
                        ...
                        className={cn({ 'bg-white !text-black ': selectedOptionValue === value }, 'h-[44px] px-6 my-1 mx-1 font-inter font-semibold text-sm  text-design-gray-900 trans-400 rounded-xl leading-[18px] ')}>
                        {text}
                    </button>
                ))
            }
