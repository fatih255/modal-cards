...
export default function Dropzone({ uploadFor }: DropzoneProps) {
  ...
  const { uploaded, assetURL } = useAppSelector(
    (state) =>
      Object({
  ...
  return (
    <div
      ...
      className={cn(
        { 'dropzone-activedrag': isDragActive },
        'dropzone flex flex-col justify-center items-center gap-5 py-7 w-full bg-white',
      )}>
      <input {...getInputProps()} />
      {isDragActive && (
        ...
        <>
          ...
          <div>
            <div className='flex flex-row flex-center child-font-inter text-sm leading-[18px] gap-1'>
              ...
              <button className='text-primary text-opacity-[0.82] hover:text-opacity-100 font-semibold underline underline-offset-2'>
                upload
              </button>
            </div>
          </div>
