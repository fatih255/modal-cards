...
function InputSelect({
  ...
  const selectAllItemsHandler = () => {
    const allItems = Array.from(document.querySelectorAll('.check-box'))
      .map((item) => {
        const itemElement = item as HTMLInputElement
        if (!itemElement.checked) (item as HTMLInputElement).checked = true
        return {
  ...
  const removeHandler = (value: string | null) => {
    ...
    const removeItem = document.querySelector(
      `.check-box:checked[data-value="${value}"]`,
    ) as HTMLInputElement
      ...
      <div className=' default-input  min-h-[38px] p-none cursor-pointer relative flex items-center'>
        {!(checkedItems.length > 0) && (
          <span
            ...
            className='text-design-gray-900 w-full h-full default-input-padding'>
            {placeholder}
          </span>
        )}
        {checkedItems.length > 0 && (
          <div className='flex gap-[2px] gap-y-[4px] flex-wrap m-1 mr-10'>
            {checkedItems.map(({ text, value }) => (
              <div
                key={`selected-item-${value}`}
                className='relative cursor-default flex basis-8 items-center gap-4 px-3 py-[6px] leading-[18px] text-sm rounded-md bg-design-gray-200'>
        ...
        <div
          onClick={activePanelHandler}
          className='flex justify-center items-center absolute inset-0 ml-[90.5%] my-auto h-full w-10'>
          <DropdownIcon />
        </div>
      </div>
      {/* dropdown panel */}
        ...
        className={cn(
          {
            'opacity-0 -translate-y-[20px] pointer-events-none':
              !activeSelectPanel,
            'translate-y-0': activeSelectPanel,
          },
        ...
        <ul className='scrollbar-style-1  pt-[14px] max-h-[305px] overflow-y-scroll '>
          <li className='flex gap-[10px] items-center cursor-pointer mx-5'>
            <input
              ...
              id='select-all'
              className='check-box w-[18px] h-[18px] my-[6px] '
              type='checkbox'
            />
            <label
              className='!my-0 !text-xs !leading-[32px] cursor-pointer hover:underline underline-offset-1'
              htmlFor='select-all'>
              {selectAllText}
            </label>
          </li>
          ...
          {items.map(({ text, value }) => (
            <li
              className='flex items-stretch w-full hover:bg-design-gray-100 cursor-pointer px-5 trans-600 bg-white'
              key={`select-${value}`}>
              <input
                checked={checkedItems.some((item) => item.value === value)}
                onChange={(e) => checkHandler(e, { text, value })}
                id={`select-${value}`}
                data-value={value}
                data-text={text}
                className='check-box'
              ...
              <label
                ...
                className='!my-0 !text-xs !leading-[32px] w-full h-full pl-[10px]  self-center  cursor-pointer'>
                {text}
              </label>
            </li>
          ))}
