...
function ModalEventListeners() {
  const { isActiveExitIntentTargetting, AfterPercentageScroll, afterXSeconds } =
    useAppSelector(
      (state) =>
        Object({
          isActiveExitIntentTargetting:
            state.modal.settings.exitIntentTargetting,
          AfterPercentageScroll: {
            isActive: state.modal.activedSettings.includes(
              'afterPercentageScroll',
            ),
  ...
  useModalEvents(
    'afterPercentageScroll',
    AfterPercentageScroll.isActive,
    AfterPercentageScroll.value,
  )
