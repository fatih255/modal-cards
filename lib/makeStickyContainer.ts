...
export default function makeStickyContainer(
  ...
  fineAdjustment: { crossTop: number; crossSticky: number } = {
    crossTop: 0,
    crossSticky: 0,
  },
  closeStickySeletor?: { selector: string; offsetCross: number },
) {
  type containerValuesType = {
  ...
  if (closeStickySeletor) {
    const closeSticky = document.querySelector(
      closeStickySeletor.selector,
    ) as HTMLElement
  ...
  document.querySelectorAll(selector).forEach((element, index) => {
    ...
    const nestedSticky =
      container.parentElement?.classList.contains(selector.replace('.', '')) ||
      container.parentElement?.parentElement?.classList.contains(
        selector.replace('.', ''),
      )
  ...
  function doFixContainers(values: containerValuesType) {
    values.forEach(({ container, sticky, rect }, index) => {
      ...
      if (
        closeStickySeletor &&
        closeStickyElement &&
        document.documentElement.scrollTop + container.offsetHeight >
          closeStickyElement.offsetTop - closeStickySeletor.offsetCross
      ) {
        nestedSticky &&
          container.children[0].setAttribute('style', `display:none `)
        !nestedSticky &&
          container.setAttribute(
            'style',
            `position:absolute; top:${
              closeStickyElement.offsetTop -
              closeStickySeletor.offsetCross -
              container.offsetHeight
            }px; left:${rect.left}px; width:${rect.width}px;`,
          )
        return
      }
      ...
      if (document.documentElement.scrollTop > stickyselection) {
        !nestedSticky &&
          container.setAttribute(
            'style',
            ` position: fixed; top:0; left:${rect.left}px; width:${rect.width}px; height:${rect.height}px; `,
          )

        if (nestedSticky) {
          container.children[0].setAttribute(
            'style',
            ` position: fixed; top:${fineAdjustment.crossTop}px; left:${rect.left}px;  height:${nestedStickyHeight}px;   z-index:9999`,
          )
          if (addednestedStickyContainers) {
            if (!container.parentElement?.offsetLeft) return

            nestedStickyBgDiv.style.width =
              container.parentElement?.offsetLeft +
              container.parentElement?.offsetWidth +
              'px'
            nestedStickyBgDiv.style.transform = `translateX(${
        ...
        if (nestedSticky) {
          container.setAttribute(
            'style',
            `pointer-events:none; position:relative; width:100%; height:${heightDivAreaSizes[index]}`,
          )
          return container.children[0].setAttribute(
            'style',
